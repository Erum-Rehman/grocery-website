...
//                             : <FadeMenu MenuTitle={user.name} classname="usericon"  options={[{name:"Log Out"},{name:"Account", href_link: "/account"} ,{name:"Order", href_link: "/order"} ]} /> 
//                          } 
//                         <div>
...
import PersistentDrawerLeft from '../Sidebar/index.js';
import PersistentDrawerRight from '../CartBag';
import { Menu as MenuIcon, ShoppingBasket as ShoppingBasketIcon, AccountCircle as AccountCircleIcon } from '@mui/icons-material';
import { Link, useNavigate } from 'react-router-dom';
import FadeMenu from '../MenuItem';
import { useSelector } from "react-redux";
    ...
    return (
        <>
            ...
            <div className="main-header">
                <Link to="/">
                    <div className="nav-logo">
                        <img src={require("../../images/logo-2.png")} alt="Logo" style={{ width: 129, height: 34 }} className='logo' />
                    </div>
                </Link>
                <ul className="nav-items">
                    <li><Link to="/">Home</Link></li>
                ...
                <div className="nav-contact-home">
                    <div className='main-header-icons'>
                        <MenuIcon onClick={handleMenuClose} className='menu' />
                        {user ? 
                            <FadeMenu MenuTitle={user.name} classname="usericon" options={[{name:"Log Out"}]} /> 
                            : <AiOutlineUser className="nav-icons" onClick={() => navigate("/register")} />
                        } 
                        <div>
                        <AiOutlineShoppingCart className="nav-icons" onClick={handleCartClose} />
